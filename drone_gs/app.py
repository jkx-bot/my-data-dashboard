...
# --- 逻辑处理：飞行监控 ---
    ...
    for _ in range(30):
        ...
        # 转换成 DataFrame
        plot_df = pd.DataFrame(st.session_state.history[-20:]) # 取最近20条
        
        with placeholder.container():
            # ... 保持之前的 m1, m2, m3 指标不变 ...
            
            st.subheader("通讯延迟 (RTT) 实时监控")
            
            # 关键修改：将 'time' 列设为索引，这样横坐标就会显示时间
            chart_data = plot_df.set_index("time")["rtt"]
            st.line_chart(chart_data)
            
            # 如果你想用更高级的图表（带坐标轴标签），也可以用这个：
